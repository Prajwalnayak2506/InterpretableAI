...
if __name__ == "__main__":
    ...
    import torch
    # 1. Force correct device detection (fixes "Torch not compiled for CUDA" on CPU machines)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"🚀 Running on: {device}")
    
    # 2. Hardcode Pythia settings (simplifies the dictionary lookups)
    n_layers = 6
    parallel_attn = True
    include_embed = True
    dtype = torch.float32

    # 3. Initialize Model WITHOUT dispatch=True (Fixes the "Stuck at 0%" hang)
    print("⏳ Loading model...")
    model = LanguageModel(
    ...
    if args.model == "EleutherAI/pythia-70m-deduped":
        model = LanguageModel(args.model, device_map=device, torch_dtype=dtype)
